        ...
        <div className="max-w-4xl mx-auto space-y-6 relative z-10">
          ...
          <form onSubmit={handleAiSearch} className="relative max-w-2xl mx-auto mt-8">
            <input
              ...
              placeholder="e.g. home painter chaiye ya carpenter..."
      ...
      <div className="max-w-6xl mx-auto px-5 space-y-16">
        ...
        {aiResults && (
          <section className="bg-slate-900/90 border border-emerald-500/30 p-6 sm:p-8 rounded-3xl space-y-6 shadow-2xl backdrop-blur-md">
            ...
            {aiResults.length === 0 ? (
              <p className="text-slate-400 text-sm py-4">No matching provider found for your request.</p>
            ) : (
                  ...
                  <div key={p.id} className="bg-slate-950 border border-slate-800 p-5 rounded-2xl space-y-3">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <h3 className="font-bold text-white text-base mt-2">{p.businessName || p.fullName || p.name}</h3>
                      </div>
                      <span className="text-xs font-black text-amber-400">★ {p.rating || '5.0'}</span>
                    </div>

                    <p className="text-xs text-slate-400 line-clamp-2">{p.bio || p.subServices || 'Verified local service provider.'}</p>
                    <p className="text-xs font-semibold text-slate-300">📍 {p.sectorArea || p.sector || 'Twin Cities'}</p>

                    <a
                      href={`https://wa.me/${p.phone || p.whatsapp}?text=Hi! I found your service via Khidmat AI.`}
                      target="_blank"
                      rel="noreferrer"
        ...
        <section className="space-y-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {CATEGORIES.map((cat) => {
              ...
              return (
                <div key={cat.id} className="group bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden hover:border-emerald-500/50 transition-all duration-300 flex flex-col justify-between shadow-xl">
                  <div>
                    ...
                    <div className="p-6 space-y-2">
                      <h3 className="font-bold text-white text-xl group-hover:text-emerald-400 transition-colors">{cat.name}</h3>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 max-w-3xl w-full max-h-[85vh] rounded-3xl overflow-hidden flex flex-col shadow-2xl">
            <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-950">
              ...
              <button onClick={() => setSelectedCategory(null)} className="w-9 h-9 rounded-full bg-slate-800 text-slate-300 hover:text-white flex items-center justify-center font-bold text-base">✕</button>
            </div>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {categoryProviders.map((p) => (
                    <div key={p.id} className="bg-slate-950 border border-slate-800 p-5 rounded-2xl space-y-3">
                      ...
                      <p className="text-xs text-slate-400 line-clamp-2">{p.bio || p.subServices || 'Verified professional service provider.'}</p>

                      <a href={`https://wa.me/${p.phone}?text=Hi! I found your profile on Khidmat.`} target="_blank" rel="noreferrer" className="block text-center w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-xl transition-all shadow-md">
                        Contact via WhatsApp
                      </a>
                    </div>
                  ))}
